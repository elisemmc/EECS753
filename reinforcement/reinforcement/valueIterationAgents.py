...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        nState = mdp.getTransitionStatesAndProbs(state, mdp.getPossibleActions(state)[0])
        print "\nnextState:" 
        print nState
        ...
        for i in range(iterations):
            ...
            for state in states:
                ...
                for action in self.mdp.getPossibleActions(state):
                    cValue = self.computeQValueFromValues(state, action)
                    if fValue == None or fValue < cValue:
                        fValue = cValue
